	...
	<div class="page">
		...
		<section class="tile">
	        ...
	        <div class="tile-header dvd dvd-btm">
	            <h1 class="custom-font"><strong>Motorbike</strong> List</h1>
	            <ul class="controls">
	                <li>
	            ...
	            <div class="table-responsive">
	                <table class="table table-custom" id="editable-usage">
	                    ...
	                    @if( $motorbike )
		                    ...
		                    <tr class="odd gradeX">
		                        <td><img src="{{ $moto->image }}" width="75" height="50" /></td>
		                        <td><a href="{{ route('admin.carriage.show', $moto->id) }}">{{ $moto->name }}</a></td>
		                        <td>{{ $moto->slug }}</td>
		                        <td>
		                        	<div class="onoffswitch labeled blue inline-block">
                                        <input type="checkbox" name="publish" cate-id={{ $moto->id }} class="onoffswitch-checkbox" id="switch{{ $moto->id }}" {{ ($moto->publish == 0) ? 'checked' : '' }}>
                                        <label class="onoffswitch-label" for="switch{{ $moto->id }}">
		                        ...
		                        <td class="actions">
		                        	<form action="{{ route('admin.carriage.destroy', $moto->id) }}" method="POST">
		                        		{{ method_field('DELETE') }}
		                        		{{ csrf_field() }}
			                        	<a href="{{ route('admin.carriage.edit', $moto->id) }}" role="button" tabindex="0" class="edit text-primary text-uppercase text-strong text-sm mr-10">Edit</a>
