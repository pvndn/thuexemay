...
@section('content')
	<div class="page">
		...
		<div class="row">
			<form action="{{ route('admin.carriage.store') }}" method="POST">
				{{ method_field('POST') }}
				{{ csrf_field() }}
		        <div class="col-md-6">
		            <section class="tile">
		        	...
		        	<section class="tile">
		                <div class="tile-body">
	                        <div class="form-group">
	                            ...
	                            <input type="text" class="form-control" name="price" placeholder="rental amount motorbike" value="{{ old('price') }}">
	                        </div>
...
@push('scripts')
	...
	<script>
		...
		function loadField() {
			...
			$('#record').append('<div class="form-group '+id+'"><div class="row"><div class="col-md-2"><a id="'+id+'" data-input="thumbnail-photo-'+id+'" data-preview="holder" class="photo btn btn-primary" onclick="return fileManager('+id+')"><i class="fa fa-picture-o"></i> Insert image </a></div><div class="col-md-8"><input id="thumbnail-photo-'+id+'" class="form-control" type="text" name="photo[]"></div><div class="col-md-2"><a class="btn btn-danger btn-rounded mb-10" href="#0" onclick="return removeField('+id+')"><i class="fa fa-trash"></i></a></div></div></div>')
		}
